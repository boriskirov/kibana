...
export interface PaletteDefinition<T = unknown> {
     ...
     * @param state  The internal state of the palette
     */
    getCategoricalColor: (series: SeriesLayer[], chartConfiguration?: ChartColorConfiguration, state?: T) => string | null;
    /**
     ...
     */
    getColorForValue?: (value: number | undefined, state: T, { min, max }: {
        min: number;
        max: number;
    }) => string | undefined;
}
...
export type RequiredPaletteParamTypes = Assign<Required<CustomPaletteParams>, {
    maxSteps?: number;
}>;
